...
def merge_datetime_and_onehot(input_path, output_path=None, drop_original=False):
    ...
    if 'cbwd' in df.columns:
        ...
        print('已对 cbwd 列做 one-hot 编码！')

    if output_path is None:
        base, ext = os.path.splitext(input_path)
        output_path = base + '_datetime_onehot.csv'
    df.to_csv(output_path, index=False)
    print(f'处理后数据已保存为: {output_path}')
    return df

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='合成 year,month,day,hour 为 datetime 列，并对cbwd做one-hot编码')
    ...
    merge_datetime_and_onehot(args.input_path, args.output, args.drop) 